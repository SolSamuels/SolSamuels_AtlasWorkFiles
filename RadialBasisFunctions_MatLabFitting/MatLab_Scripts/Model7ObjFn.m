...
function ObjFn = Model7ObjFn(x, r, rhoRef, Ne, w1, w2, w3, w4, w5, r1, r2, rCut)
...
F5_d   = F5_a + F5_c;
F5_e   = (F5_d).*(F5_d);
F5     = E0*F5_e.*exp(-2*delta*r);


%F5  = E0*((pp-(4*Zeff*r)+((2*Zeff*r)/3).^2).^2).*exp(-2*delta*r);
...
%----Fifth term, 3s shell structure----
fun5     = @(r)(r.^2).*((6-(4*Zeff*r)+((2*Zeff*r)/3).^2).^2).*exp(-2*delta*r);
int_fun5 = integral(fun5,0,Inf);

rsq_den_fit=@(r)r.^2.*(A0.*(exp(-2.*Zeff.*r)) +...
    (r>0).*(B0.*((r).^(2.*((r==0).*1 + (r>0).*(beta + bb.*r)))).*(exp(-2.*alpha.*(r)))) +...
    C0.*((mm-(Zeff*r)).^2).*exp(-2*gammaf*r) + D0.*(Zeff*r).^2.*exp(-2*eta*r) +...
    E0*((pp-(qq*Zeff*r)+(tt*Zeff*r).^2).^2).*exp(-2*delta*r));
...
KatoNum = Zeff*A0 + C0*mm*Zeff + gammaf*C0*mm^2 + E0*qq*pp*Zeff + delta*E0*pp^2;
KatoDen = Zeff*(A0 + C0*mm^2 + E0*pp^2);
