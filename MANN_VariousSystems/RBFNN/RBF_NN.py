...
start_dcy = 0.0001            # L2 weight decay parameter
end_dcy = start_dcy         # Change if want different L2 weight decay parameter in short-range
...
# Define requested states: (name, charge, model)
ATOM_A_STATES = [
...
# ---------------------------------
# PRINT OUT OF RBF_NN PARAMETERS:
# ---------------------------------
...
# ---------------------------------
# LIST OF INTERNUCLEAR SEPARATIONS TO EVALUATE
# ---------------------------------

# Internuclear separations (in Angstrom)
# Must be in string format and must match file naming convention
# Build separation list automatically from directories
sep_dict = []
...
# Header for mas_data excel file output which includes results
mas_header = [
    'R (A)',
    'R (a.u.)',
    'batch size',
...
#     Portions may need to be changed between molecules (number of weights, calling of parameters, naming...)
...
# LOSS FUNCTION:
# ---------------------------------
#     The following function designates the loss function which is called on by the RBF-NN during training
...
def loss_fit(rho_true, rho_pred):
    ...
    reduced_loss = (
        loss_fitparam *
        tf.reduce_mean(tf.square(rho_true - rho_pred))

        + loss_chargeparam *
        tf.abs(A_charge + B_charge)

        + loss_AtomAsum *
        tf.abs(weight_sum(A_weights) - 1)

        + loss_AtomBsum *
        tf.abs(weight_sum(B_weights) - 1)
    )
...
def AtomA_sum(rho_true, rho_pred):
    ...
    A_weights, _ = split_weights(weights)
...
def AtomB_sum(rho_true, rho_pred):
    ...
    _, B_weights = split_weights(weights)
    ...
    try:
        ...
        with open(main_path + f"{key_str}A/{molecule}_{key_str}A_Den{coarseness}.out", "r") as file:
            for line in file:
                ...
                column3 = float(realvals[2]) - shift
